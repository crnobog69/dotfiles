...
status_repository() {
    local repo=$1
    gum style --border normal "Статус репозиторијума: $(gum style --foreground "${YELLOW}" "$repo")"
    cd "$repo" || exit
    
    gum spin --spinner.foreground="${YELLOW}" --title " Провера статуса..." --spinner dot -- sleep 1
    git status
}

log_repository() {
    local repo=$1
    gum style --border normal "Комит лог репозиторијума: $(gum style --foreground "${YELLOW}" "$repo")"
    cd "$repo" || exit
    
    gum spin --spinner.foreground="${YELLOW}" --title " Учитавање комит лога..." --spinner dot -- sleep 1
